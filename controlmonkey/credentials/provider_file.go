...
const (
	...
	FileCredentialsEnvVarFile = "CONTROL_MONKEY_CREDENTIALS_FILE"
	...
	FileCredentialsEnvVarProfile = "CONTROL_MONKEY_CREDENTIALS_PROFILE"
)

var (
	// ErrFileCredentialsLoadFailed is returned when the provider is unable to load
	// credentials from the credentials file.
	ErrFileCredentialsLoadFailed = errors.New("controlmonkey: failed to load credentials file")

	// ErrFileCredentialsNotFound is returned when the loaded credentials
	// are empty.
	ErrFileCredentialsNotFound = errors.New("controlmonkey: credentials file or profile is empty")
)
...
//   - Linux/Unix : $HOME/.controlmonkey/credentials
//   - Windows    : %USERPROFILE%\.controlmonkey\credentials
func DefaultFilename() string {
	return filepath.Join(userHomeDir(), ".controlmonkey", "credentials")
}
