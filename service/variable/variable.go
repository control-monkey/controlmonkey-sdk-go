...
func (s *ServiceOp) ListVariables(ctx context.Context, input *ListVariablesInput) (*ListVariablesOutput, error) {
	...
	gs, err := variablesFromHttpResponse(resp)
	if err != nil {
		return nil, err
	}

	return &ListVariablesOutput{Variables: gs}, nil
}
...
func (s *ServiceOp) DeleteVariable(ctx context.Context, input *DeleteVariableInput) (*DeleteVariableOutput, error) {
	path, err := uritemplates.Expand("/variable/{variableId}", uritemplates.Values{
		"variableId": controlmonkey.StringValue(input.VariableId),
	})
	if err != nil {
	...
	output := new(DeleteVariableOutput)
	return output, nil
}
