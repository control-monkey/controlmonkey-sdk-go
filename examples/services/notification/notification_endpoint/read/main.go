...
func main() {
	...
	t, err := svc.ReadNotificationEndpoint(ctx, notificationEndpointId)
	if err != nil {
		log.Fatalf("Control Monkey: failed to read notification endpoint: %v", err)
	}

	// Output notification endpoint, if exists.
	if t != nil {
