...
func main() {
	...
	//scopeId := ""
	t, err := svc.ListEventSubscriptions(ctx, "organization", nil)
	if err != nil {
		log.Fatalf("Control Monkey: failed to read event subscriptions: %v", err)
	}

	if t != nil {
		log.Printf("Event subscriptions: %s",
