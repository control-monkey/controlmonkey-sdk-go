...
func main() {
	...
	out, err := svc.CreateControlPolicyGroup(ctx, &control_policy_group.ControlPolicyGroup{
		...
		ControlPolicies: []*control_policy_group.ControlPolicy{
			{
				...
				Severity:        controlmonkey.String("high"),
			},
			{
				ControlPolicyId: controlmonkey.String("pol-wodtywo3dn"),
				Severity:        controlmonkey.String("low"),
			},
		},
	})

	if err != nil {
		log.Fatalf("Control Monkey: failed to create ControlPolicyGroup: %v", err)
	}

	// Output ControlPolicyGroup, if was created.
	if out != nil {
